...
class RoutesController extends GetxController {
  ...
  setCheckedPriority(int index) {
    ...
    switch (index) {
      case 1:
        propertyName = "route_passes_count";
        break;
      case 2:
        propertyName = "route_percent";
        break;
      case 4:
        propertyName = "route_comments_count";
        break;
      case 5:
  ...
  loadRoutes() async {
    var result = await OriApi.getCoachRoutes(64);
    ...
    mainController.lastRoute = int.parse(routes[routes.length - 1]
                ["route_generated_name"]
            .substring(routes[routes.length - 1]["route_generated_name"]
                    .lastIndexOf("-") +
                1))
        .obs;
    mainController.lastRoute.value++;
    setFilterRoutes();
  }
  ...
  setFilterRoutes() {
    ...
    switch (checkedSalesFilter.value) {
      case "<100":
        filteredRoutes.value = filteredRoutes
            .where((item) => int.parse(item["route_cash_count"] ?? "0") <= 100)
            .toList();
        filteredCount.value++;
        break;
      case "100-1K":
        filteredRoutes.value = filteredRoutes
            .where((item) =>
                int.parse(item["route_cash_count"] ?? "0") > 100 &&
                int.parse(item["route_cash_count"] ?? "0") <= 1000)
            .toList();
        filteredCount.value++;
        break;
      case "1K>":
        filteredRoutes.value = filteredRoutes
            .where((item) => int.parse(item["route_cash_count"] ?? "0") > 1000)
            .toList();
        filteredCount.value++;
        break;
      default:
    ...
    switch (checkedPassesFilter.value) {
      case "<100":
        filteredRoutes.value = filteredRoutes
            .where(
                (item) => int.parse(item["route_passes_count"] ?? "0") <= 100)
            .toList();
        filteredCount.value++;
        break;
      case "100-1K":
        filteredRoutes.value = filteredRoutes
            .where((item) =>
                int.parse(item["route_passes_count"] ?? "0") > 100 &&
                int.parse(item["route_passes_count"] ?? "0") <= 1000)
            .toList();
        filteredCount.value++;
        break;
      case "1K>":
        filteredRoutes.value = filteredRoutes
            .where(
                (item) => int.parse(item["route_passes_count"] ?? "0") > 1000)
            .toList();
        filteredCount.value++;
        break;
      default:
    ...
    switch (checkedLengthFilter.value) {
      case "<2":
        filteredRoutes.value = filteredRoutes
            .where((item) => double.parse(item["route_length"].toString()) <= 2)
            .toList();
        filteredCount.value++;
        break;
      case "2-4":
        filteredRoutes.value = filteredRoutes
            .where((item) =>
                double.parse(item["route_length"].toString()) > 2 &&
                double.parse(item["route_length"].toString()) <= 4)
            .toList();
        filteredCount.value++;
        break;
      case "4-7":
        filteredRoutes.value = filteredRoutes
            .where((item) =>
                double.parse(item["route_length"].toString()) > 4 &&
                double.parse(item["route_length"].toString()) <= 7)
            .toList();
        filteredCount.value++;
        break;
      case "7>":
        filteredRoutes.value = filteredRoutes
            .where((item) => double.parse(item["route_length"].toString()) > 7)
            .toList();
        filteredCount.value++;
        break;
      default:
    ...
    filteredRoutes.forEach((element) {
      sumPasses.value =
          int.parse(element["route_passes_count"] ?? "0") + sumPasses.value;
      sumSales.value += int.parse(element["route_cash_count"] ?? "0");
      sumTips = int.parse(element["route_percent"] ?? "0") + sumTips;
    });
    try {
